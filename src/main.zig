...
const GameBoard = struct {
    ...
    pub fn init(alloc: std.mem.Allocator, width: u16, height: u16) GameBoard {
        ...
        return .{
            ...
            .food = .{
                .pos = .{
                    .x = @as(i16, @intCast(rand.intRangeAtMost(u16, 0, cols))),
                    .y = @as(i16, @intCast(rand.intRangeAtMost(u16, 0, rows))),
                },
                .color = FOOD_COLOR,
            },
        };
...
fn pollPlayerEvents(board: *GameBoard) void {
    if (board.player.pos.x == board.food.pos.x and board.player.pos.y == board.food.pos.y) {
        board.player.size += 1;
        board.food.pos = .{
            .x = @as(i16, @intCast(rand.intRangeAtMost(u16, 0, board.cols))),
            .y = @as(i16, @intCast(rand.intRangeAtMost(u16, 0, board.rows))),
        };
    }
}
